...
describe('useSearch', () => {
  ...
  it('debounces search by 300ms', async () => {
    ...
    expect(searchApi.searchSemantic).toHaveBeenCalledWith('python', 20)
  })
  ...
  it('updates domain filter via URL param', () => {
    const { result } = renderHook(() => useSearch(), { wrapper })
    act(() => { result.current.setDomain('python') })
    expect(result.current.domain).toBe('python')
  })

  it('setDomain deselects when empty string passed', () => {
    const { result } = renderHook(() => useSearch(), { wrapper })
    act(() => { result.current.setDomain('python') })
    act(() => { result.current.setDomain('') })
